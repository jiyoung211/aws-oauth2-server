...
public class QuoteService
{
  ...
  @Path("/view-quote")
  @Produces("text/html")
  public Response viewQuoteAutoTest(@Context ClientTokenContext context)
  {
   ...
   * 
   * 
   * AuthorizationCodeGrant codeGrant = new AuthorizationCodeGrant(code,
   * ui.getBaseUriBuilder().path("viewquote").build());
   * 
   * ClientAccessToken accessToken = manager.getAccessToken(codeGrant); if
   * (accessToken == null) { return
   * redirectToFailureHandler("No Access Token available."); } String authHeader
   * = manager.createAuthorizationHeader(accessToken);
   * apiService.replaceHeader(HttpHeaders.AUTHORIZATION, authHeader);
   * 
   * try { return apiService.get(); } catch (NotAuthorizedException ex) { String
   * refreshToken = accessToken.getRefreshToken(); if (refreshToken != null) {
   * // retry once
   * 
   * // refresh the token accessToken = manager.getRefreshedAccessToken();
   * 
   * authHeader = manager.createAuthorizationHeader(accessToken);
   * apiService.replaceHeader(HttpHeaders.AUTHORIZATION, authHeader);
   * 
   * // try to access the end user resource again return apiService.get();
   * 
   * } else { throw ex; }
   * 
   * }
