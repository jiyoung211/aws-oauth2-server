...
public class CallTableManyRows extends CallProcManyRows
{
    ...
    @Override
    public ArrayList<HashMap<String, Object>> callProc(String procNm,
    ...
    public ArrayList<HashMap<String, Object>> callTable(String procNm,
            Object... args) throws SQLException
    {
        ...
        try
        {
            ...
            NamedParameterStatement nstmt = createNamedParameterStatement(
                    dbConn, procNm, args);
            // PreparedStatement pstmt = createPreparedStatement(dbConn, procNm,
            // args)
            // ;
            ...
            if (found)
            {
                ...
                do
                {
                    ...
                    ++idx;
                    HashMap<String, Object> rowHashMap = (HashMap) rsMany
                            .get(idx);
                    int i;
                    for (i = 1; i <= colCount; ++i)
                    {
                                ...
                                && metaData.getColumnType(i) != 12)
                        {
                            if (metaData.getColumnType(i) != 4
                                    && metaData.getColumnType(i) != 5
                                    && metaData.getColumnType(i) != -6)
                            {
                                    ...
                                    } else
                                    {
                                        if (metaData.getColumnType(i) != -4)
                                        {
                                            throw new IllegalArgumentException(
                                                    "Invalid datatype returned from database: "
                                                            + metaData
                                                                    .getColumnTypeName(i));
                                        }
        ...
        {
            dbMessage = var42.getErrorCode() + " CallProcManyRows:"
                    + debugProcCall + " SQLException " + var42.getMessage();
            dbError = true;
            throw var42;
        } catch (Throwable var43)
        {
            dbMessage = "Unknown Exception " + this.getClass().getName() + ": "
                    + debugProcCall + var43.getMessage();
            dbError = true;
        ...
        } finally
        {
            if (dbConn != null)
            {
                ...
                {
                    System.out
                            .println("Connection close " + var40.getMessage());
                }
            }
    ...
    public ArrayList<HashMap<String, Object>> toLowerCaseColumnName(
            ArrayList<HashMap<String, Object>> upperMap)
    {
    ...
    private NamedParameterStatement createNamedParameterStatement(
            Connection dbConn, String procNm, Object... args)
    {
        ...
        case "s_access_token":
            // spSelectDAO.callProc("s_oauth_accesstoken", null,
            // resourceOwnerSubject
            // != null ? resourceOwnerSubject.getLogin() : null,
            // c.getClientId()) ;
            sb.append(" SELECT t.* FROM BearerAccessToken t ");
        ...
        case "s_oauth_refreshtoken":
            // spSelectDAO.callProc("s_oauth_refreshtoken", null,
            // resourceOwnerSubject
            // != null ? resourceOwnerSubject.getLogin() : null,
            // c.getClientId()) ;
            // spSelectDAO.callProc("s_oauth_refreshtoken", refreshTokenKey,
            // null,
            // null) ;
        ...
        case "s_oauth_serverauthcodegrant":
            // spSelectDAO.callProc("s_oauth_serverauthcodegrant",
            // c.getClientId(),
            // resourceOwnerSubject != null ? resourceOwnerSubject.getId() :
            // null) ;
            // spSelectDAO.callProc("s_oauth_serverauthcodegrant", code,
            // null, null) ;
            if (args.length == 2)
            {
                // resultSet =
                // spSelectDAO.callProc("s_oauth_serverauthcodegrant",
                // c.getClientId(),
                // resourceOwnerSubject != null ?
                // resourceOwnerSubject.getId() : null) ;
        ...
        case "s_oauth_usersubject_roles":
            // spSelectDAO.callProc("s_oauth_usersubject_roles",
            // userSubject.getId())
            // ;
