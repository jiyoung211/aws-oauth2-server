...
public class LDAPUtils
{
  ...
  private int connect()
  {
    try
    {
      ...
      ChaiConfiguration chaiConfig = new ChaiConfiguration(ldapURL, ldapAdminDN, ldapAdminPW) ;
    ...
    {
      log.error("LDAP configuration is not correct. Please check its connection details.", iae) ;
      return -1 ;
    }
  ...
  public LDAPUser authenticate(String nickName, String password)
  {
    ...
    if (user != null)
    {
      ...
      {
        /*
         * User is found so lets assume he is valid and then check all other
         * conditions.
         */
        bAuthenticated = true ;
        Date loginExpirationTime = user.readDateAttribute(LOGIN_EXPIRATION_TIME) ;
        ...
        else if (loginExpirationTime != null && Calendar.getInstance().getTime().after(loginExpirationTime))
        {
        ...
        else
        {
          ...
          ldapuser.setBrokerNumber(user.readStringAttribute(BROKER_OFFICE)) ;
        }
      }
      catch (ChaiUnavailableException cue)
      {
      ...
      {
        log.error("LDAP Chai operation could notbe performed on user object.", coe) ;
        bAuthenticated = false ;
      }
    }
  ...
  /**
   * Find user with its nickName/username and also check is required parameter.
   * 
   * @param nickName
   * @return
   */
  ...
  {
    boolean bFound = false ;
    String userDN = "cn=" + userCN + ",ou=" + ldapContainer + ",o=" + ldapRoot ;
    ChaiUser user = null ;
    try
    {
    ...
    if (!bFound)
    {
      /* If user is not valid it falls into here to search on the authNickName. */
      ...
      try
      {
        ...
        Map<String, Map<String, String>> searchResult = provider.search("ou=" + ldapContainer + ",o=" + ldapRoot, searchHelper) ;
  ...
  /*
   * createUser - Attempt to create a new LDAP user object - No check is done to
   * ensure the user doesn't already exist, this method will fail (return -1) if
   * it already exists
   * 
   * @param userSN The Surname (SN) is required because it must be set when the
   * object is created
   * 
  ...
  public int createUser(String userCN, String userSN)
  {
    ...
    try
    {
      ...
      provider.createEntry(userDN, ChaiConstant.OBJECTCLASS_BASE_LDAP_USER, props) ;
    }
    /* If an error is encountered during the operation */
    catch (ChaiOperationException coe)
    {
   ...
   * check is done to ensure the user or group is valid - If the user is already
   * in the group, this method will not attempt to add the user to the group,
   * and will return 0 - The distinguished name (DN) of both the user and the
   * group are required - If an error occurs this method returns -1
   */
  public int addGroup(String userDN, String groupDN)
  {
    ...
    try
    {
      ...
      {
        if (groupArray[i].toString().equalsIgnoreCase((group.toString())))
        {
          return 0 ;
        }
      }
   ...
   * No check is done to ensure the user or group is valid - If the user is not
   * already in the group, this method will not attempt to remove the user from
   * the group, and will return 0 - The distinguished name (DN) of both the user
  ...
  /*
   * addObjectClass - Used when initially creating a user to add object classes
   * to the user
   */
   ...
   * This is merely a convenience method to save the user from having to specify
   * the multiValueInd
   */
   ...
   * method will create it and set its value - If the attribute already exists,
   * this method will merely update its value - If this is a multi-valued
   * attribute, and you are attempting to append values to the attribute, the
   * multiValueInd should be set to "Y" - If an error occurs this method returns
   * -1
   */
  public int setAttribute(String userDN, String attrName, String attrValue, String multiValueInd)
  {
    ...
    try
    {
      ...
      /*
       * A null in the attribute means that it does not exist, and needs to be
       * added
       */
      if (checkAttrValue == null || multiValueInd == "Y")
      {
        /*
         * If you try to create an attribute with a null value, the update will
         * fail
         */
        if (attrValue != null)
        {
  ...
  public String getDnFromCn(String userCN)
  {
    ...
    try
    {
      ...
      ChaiEntry searchRoot = ChaiFactory.createChaiEntry(searchRootDN, provider) ;

      ChaiEntry[] resultArray = {} ;

      resultArray = searchRoot.search("cn=" + userCN).toArray(resultArray) ;
  ...
  /*
   * countMatchingEntries - given a base DN (empty string if the entire tree is
   * to be searched) and a valid LDAP search filter, will return a value
   ...
   * regionGroup may contain an LDAP group that a new user id needs to be added
   * to for that region, i.e. volume, training, prop support. Some regions won't
   * have a group, i.e. dev, acceptance, production
   */
  ...
  /*
   * generateRandomPassword - generates a random password based on the passed in
   * passwordMask - the mask should contain characters where characters are
   * required and digits where digits are required eg. "aaaaaaa0" will generate
   * 7 random characters followed by 1 random digit
   */
  public String generateRandomPassword(String passwordMask)
  {
    ...
    {
      log.error("Alogorithm is not available to generate random password.", nsae) ;
      return null ;
    }
  ...
  /*
   * isLocked - checks to see if the user's account is currently locked (usually
   * due to intruder detection) - if user is locked, returns a 1 - if user is
   * not locked, returns a 0 - if an LDAP error occurs -1 will be returned
   */
  ...
  public String loadProperty(String prop)
  {
    ...
    try
    {
      ...
      propertiesPath = (String) initCtx.lookup("java:comp/env/propertiesPath") ;
    }
    catch (NamingException ne)
    {
      log.fatal("Naming exception occured while retriving properties path.", ne) ;
    }
    ...
    try
    {
      ...
      String path = propertiesPath != null ? (propertiesPath + propertiesFileName) : propertiesFileName ;
