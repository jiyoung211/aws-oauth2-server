...
//****Utility 메서드 끝	
	...
	public ModelAndView authorize(RequestAuthVO vo, HttpServletResponse response, HttpServletRequest request)
	...
	public String authorizePost(Model model, RequestAuthVO rVO, HttpServletRequest request,
...
//		if (!rVO.getRedirect_uri().equals(cVO.getRedirect_uri())) {
		if (!cVO.getRedirect_uri().contains(rVO.getRedirect_uri()))
		{
		...
		} else if (response_type.equals(OAuth2Constant.RESPONSE_TYPE_TOKEN))
		{
			...
			{
				tokenVO = new ResponseAccessTokenVO(tVO.getAccess_token(), tVO.getToken_type(), tVO.getExpires_in(),
						tVO.getRefresh_token(), rVO.getState(), tVO.getCreated_at());
			} else
			{
				// OAuth2AccessToken 클래스 기능을 이용해 정해진 규칙에 의해 생성함.
				tokenVO = new ResponseAccessTokenVO(
						this.tokenService.generateAccessToken(cVO.getClient_id(), cVO.getClient_secret(),
								uVO.getUserid(), uVO.getPassword()),
	...
	private ResponseAccessTokenVO accessTokenServerFlow(RequestAccessTokenVO ratVO, HttpServletRequest request)
			throws OAuth2Exception
	{
...
//		if (!ratVO.getRedirect_uri().equals(cVO.getRedirect_uri()))
		if (!cVO.getRedirect_uri().contains(ratVO.getRedirect_uri()))
		{
			throw new OAuth2Exception(400, OAuth2ErrorConstant.NOT_MATCH_REDIRECT_URI);
		}
		...
		} else
		{
			...
			resVO.setAccess_token(this.tokenService.generateAccessToken(cVO.getClient_id(), cVO.getClient_secret(),
					uVO.getUserid(), uVO.getPassword()));
		}
	...
	// grant_type이 authorization_code일 때
	private ResponseAccessTokenVO refreshTokenFlow(RequestAccessTokenVO ratVO, HttpServletRequest request)
			throws OAuth2Exception
	{
		...
		ResponseAccessTokenVO resVO = new ResponseAccessTokenVO(tVO.getAccess_token(), tVO.getToken_type(),
