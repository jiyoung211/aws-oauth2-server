...
public class OAuth2DataProviderImpl extends AbstractOAuthDataProvider implements
        AuthorizationCodeDataProvider
{
    ...
    public void checkRequestedScopes(Client client, List<String> requestedScopes)
    {
         ...
         */
        if (grantType != null
                && !requestedScopes.contains(OAuth2Constants.API_ACCESS_SCOPE))
        {
            System.out
                    .println("throw new OAuthServiceException(\"Required scopes are missing\")");
    ...
    @Override
    public Client doGetClient(final String clientId)
    ...
    {
        /*
         * Do nothing as we are using LDAP for third party client and their
         * staff so no need to remove in any oauth2 flow.
         */
    }
    ...
    @Override
    public List<ServerAccessToken> getAccessTokens(final Client c,
            final UserSubject sub)
    {
        return oauth2DAO.selectAccessTokens(c, sub);
    }

    @Override
    public List<RefreshToken> getRefreshTokens(final Client c,
            final UserSubject sub)
    {
    ...
    @Override
    public ServerAccessToken getAccessToken(final String accessToken)
            throws OAuthServiceException
    {
    ...
    @Override
    protected void linkRefreshTokenToAccessToken(final RefreshToken rt,
            final ServerAccessToken at)
    {
        super.linkRefreshTokenToAccessToken(rt, at);
        oauth2DAO.updateRefreshTokenInAccessToken(rt.getTokenKey(),
                at.getTokenKey());
    }
            ...
            AccessTokenRegistration atReg)
    {
        ...
        // injected
        // from another entity
        // this can be the case when using multiple cmt dataProvider operation
        // in a
        ...
        {
            at.setExtraProperties(new HashMap<String, String>(at
                    .getExtraProperties()));
        }
    ...
    protected void saveAccessToken(final ServerAccessToken serverToken)
    {
        ...
        {
            OAuthPermission permSaved = oauth2DAO.selectOAuthPermission(perm
                    .getPermission());
            if (permSaved != null)
            {
                perms.add(permSaved);
        ...
        {
            UserSubject sub = oauth2DAO.selectUserSubject(serverToken
        ...
        {
            serverToken.setClient(doGetClient(serverToken.getClient()
                    .getClientId()));
        }
    ...
    protected ServerAccessToken doRefreshAccessToken(Client client,
            RefreshToken oldRefreshToken, List<String> restrictedScopes)
    {
        ServerAccessToken at = super.doRefreshAccessToken(client,
                oldRefreshToken, restrictedScopes);
    ...
    protected ServerAuthorizationCodeGrant doCreateCodeGrant(
            AuthorizationCodeRegistration reg) throws OAuthServiceException
    {
    ...
    protected void saveCodeGrant(final ServerAuthorizationCodeGrant grant)
    {
        ...
        {
            UserSubject sub = oauth2DAO.selectUserSubject(grant.getSubject()
                    .getId(), null);
            if (sub == null)
            {
    ...
    {
        /* Server code grant is currently not supported. */
        ServerAuthorizationCodeGrant serverAuthorizationCodeGrant = oauth2DAO
                .selectServerAuthorizationCodeGrantInfo(code);
        System.out.println("##1 serverAuthorizationCodeGrant "
                + serverAuthorizationCodeGrant);
        if (serverAuthorizationCodeGrant != null)
            oauth2DAO.deleteServerAuthorizationCodeGrant(code);
        System.out.println("##2 serverAuthorizationCodeGrant "
                + serverAuthorizationCodeGrant);
    ...
    @Override
    public List<ServerAuthorizationCodeGrant> getCodeGrants(final Client c,
            final UserSubject subject) throws OAuthServiceException
    {
        /* Server code grant is currently not supported. */
    ...
    {
        List<String> registeredScopes = getPermissionMap().entrySet().stream()
                .map(e -> e.getValue().getPermission())
        ...
        extraProperties.put(OAuth2Constants.BROKER_NUMBER,
                user.getBrokerNumber());

        UserSubject clientUserSubject = new UserSubject(user.getUsername(),
                user.getUsername());
        clientUserSubject
