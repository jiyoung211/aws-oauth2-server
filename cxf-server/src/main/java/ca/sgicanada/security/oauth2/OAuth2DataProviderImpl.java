...
public class OAuth2DataProviderImpl extends AbstractOAuthDataProvider implements AuthorizationCodeDataProvider
{
	...
	public void checkRequestedScopes(Client client, List<String> requestedScopes)
	{
		...
		/*
		 * If required scope is not included in approved scopes for access token request
		 * then do not generate access token.
		 */
	...
	// "\n")) ;
	// throw new
	// OAuthServiceException("Could not found client due to unavailablity of
	// identity server.")
	// ;
	// }
	// return client ;
	...
	public Client doGetClient(final String clientId) throws OAuthServiceException
	{
		...
		if (client.getRedirectUris().size() == 0)
		{
			...
			uris.add("http://13.125.39.12:7070/oauthServer.jsp");
			uris.add("http://13.125.39.12:7070/login.jsp");
			uris.add("http://13.125.39.12:7070/");
			uris.add("http://ec2-52-78-1-158.ap-northeast-2.compute.amazonaws.com:7070/oauthServer.jsp");
			uris.add("http://ec2-52-78-1-158.ap-northeast-2.compute.amazonaws.com:7070/login.jsp");
			uris.add("http://ec2-52-78-1-158.ap-northeast-2.compute.amazonaws.com:7070/");
	...
	{
		/*
		 * Do nothing as we are using LDAP for third party client and their staff so no
		 * need to remove in any oauth2 flow.
		 */
	}
	...
	{
		List<String> registeredScopes = getPermissionMap().entrySet().stream().map(e -> e.getValue().getPermission())
